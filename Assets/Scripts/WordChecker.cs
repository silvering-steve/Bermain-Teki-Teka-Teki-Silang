...
public class WordChecker : MonoBehaviour
{
    ...
    private void SquareSelected(string letter, Vector3 squarePosition, int squareIndex) {
        //this gonna happen if the selected square is the first square which we selected
        if(_assignedPoints == 0) {
            ...
            _word += letter;
            CheckWord();
        } // if it's any square after
        else {
            //if the following square is currently lying on the right which we selected from here
    ...
    private void CheckWord() {
        ...
        foreach(var searchingWord in currentGameData.selectedBoardData.SearchWords) {
            if(_word == searchingWord.Word) {
                ...
                _word = string.Empty;
                _correctSquareList.Clear(); //clear the list after calling the event not before because otherwise, the list of the indexes will be empty
                return;
            }
        }
    }
    ...
    private Ray SelectRay(Vector2 firstPosition, Vector2 secondPosition) {
        var direction = (secondPosition - firstPosition).normalized; //just have a normalized direction which will be 1 or -1, so we will know if we're gonna go up or down
