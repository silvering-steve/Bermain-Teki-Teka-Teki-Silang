...
[CanEditMultipleObjects] //if there will be more than one data files created in this project, we can edit them separately
[System.Serializable]
public class BoardDataDrawer : Editor {
    //Grab our instance of Board Data Class and then reference to it
    private BoardData GameDataInstance => target as BoardData;
    private ReorderableList _dataList;

    private void OnEnable() {
                                            //propertyName, listLabel
        InitializeReordableList(ref _dataList, "SearchWords", "SearchingWords");
    }
    ...
    public override void OnInspectorGUI() {
        ...
        EditorGUILayout.Space();
        _dataList.DoLayoutList(); //display list
        ...
        for(var x = 0; x < GameDataInstance.Columns; x++) {
            ...
            for(var y = 0; y < GameDataInstance.Rows; y++) {
                if(x >= 0 && y >= 0) {
                    ...
                    if(GameDataInstance.Board[x].Row[y].Length > 1) { //if we put more than 1 character in the specific field
        ...
        if(GUILayout.Button("Fill Up with Random")) {
            for(int i = 0; i < GameDataInstance.Columns; i++) {
                for(int j = 0; j < GameDataInstance.Rows; j++) {
                    ...
                    if(errorCounter == 0) { //if the certain field is empty
