...
public class WordsGrid : MonoBehaviour
{
    ...
    private void SetSquaresPosition() {
        ...
        var offset = new Vector2 {
            //set the member of the vector 2
            ...
            foreach (var squares in currentGameData.selectedBoardData.Board)
            {
                foreach(var squareLetter in squares.Row) {
                    ...
                    if(normalLetterData.image == null || selectedLetterData.image == null) {
                        ...
                        _squareList[_squareList.Count - 1].GetComponent<Transform>().position = new Vector3(0f, 0f, 0f); //set the position
                        _squareList[_squareList.Count - 1].transform.localScale = squareScale; //set the squareScale
    ...
    private Vector3 GetSquareScale(Vector3 defaultScale) {
        ...
        var adjustment = 0.01f; //value -> how much we will scale down or up specific square based on the iteration
    ...
    //to check if the whole board will be fit on our screen, if not it's going to scale down all of the squares until they can all be displayed on our screen
    private bool ShouldScaleDown(Vector3 targetScale) {
        var squareRect = gridSquarePrefab.GetComponent<SpriteRenderer>().sprite.rect;
        //we'll gonna capture the size of our sprite
        var squareSize = new Vector2(0f, 0f);
