...
public class WordsGrid : MonoBehaviour
{
    ...
    private void SetSquaresPosition()
    {
        ...
        var squareTran = _squareList[0].GetComponent<Transform>();
    
        var offset = new Vector2
        {
            x = (squareRect.width * squareTran.localScale.x + squareOffset) * 0.01f,
            y = (squareRect.height * squareTran.localScale.y + squareOffset) * 0.01f
        };
        ...
        {
            if (rowNumber + 1 > currentGameData.selectableBoardData.rows)
            {
                columnNumber++;
                rowNumber = 0;
            }
    ...
    private Vector2 GetFirstSquarePosition()
    {
        ...
        squareSize.y = squareRect.width * squareTransform.localScale.y;

        var midWidthPosition = (((currentGameData.selectableBoardData.columns - 1) * squareSize.x) / 2) * 0.01f;
        var midWidthHeight = (((currentGameData.selectableBoardData.rows - 1) * squareSize.y) / 2) * 0.01f;
    ...
    private void SpawnGridSquares()
    {
        if (currentGameData != null)
        {
            ...
            foreach (var squares in currentGameData.selectableBoardData.board)
            {
                foreach (var squareLetter in squares.row)
                {
                    ...
                    {
                        Debug.Log("Add more data to " + squareLetter);
                    ...
                    else
                    {
                        ...
                        _squareList[_squareList.Count -1 ].GetComponent<GridSquare>().SetSprite(normalLetterData, 
                            selectedLetterData, correctLetterData);
    ...
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        var midWidthPosition = ((currentGameData.selectableBoardData.columns * squareSize.x) / 2) * 0.01f;
        var midWidthHeight = ((currentGameData.selectableBoardData.rows * squareSize.y) / 2) * 0.01f;

        startPosition.x = (midWidthPosition != 0) ? midWidthPosition * -1 : midWidthPosition;
        startPosition.y = midWidthHeight;
        
        return (startPosition.x < GetHalfScreenWidth() * -1 || startPosition.y > topPosistion);
    }
